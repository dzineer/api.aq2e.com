...
/*
 * Registeredlinkfacade.php -
 *
 */

class Registeredlinkfacade
{
	var $debug = false;
	...
	function __construct($Domain = '127.0.0.1') {
		...
		$this->CI->load->model('CampaignMonitorLink_model', 'RegisteredLink');
	...
	function get_all_registered_links() {
		$links = $this->CI->RegisteredLink->get_all_monitor_links();
	    return $links;
    }

    function search( $hash ) {
        $link = $this->CI->RegisteredLink->get_monitor_link( $hash );
        return $link;
    }
